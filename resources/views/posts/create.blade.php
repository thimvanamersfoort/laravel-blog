    ...
    <div class="max-w-2xl mx-auto flex flex-col min-h-screen">
      ...
      <form action="/posts" method="post" class="space-y-8 mt-12">
        ...
        <div class="relative border border-gray-300 rounded-md px-3 py-2 shadow-sm focus-within:ring-1 focus-within:ring-indigo-600 focus-within:border-indigo-600 sm:w-[80%]">
          ...
          <input type="text" name="title" id="title" required class="block border-0 p-0 text-gray-900 placeholder-gray-500 focus:ring-0 sm:text-sm w-full" value="{{ old('title') }}" placeholder="Title of your blogpost">
        </div>
        ...
        <div class="relative">
          ...
          <div class="mt-1">
            <textarea rows="20" name="content" id="content" required class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-lg" value="{{ old('content') }}" placeholder="content of your blogpost. Text only."></textarea>
          </div>
        </div>

        <div class="flex flex-row items-center px-5 py-3 rounded-lg border-gray-400 border-dashed border max-w-fit">
          <img class="h-10 w-10 rounded-full" src="https://avatars.dicebear.com/api/initials/{{ substr(Auth::user()->name, 0, 2) }}.svg?background=%23000" alt="">
