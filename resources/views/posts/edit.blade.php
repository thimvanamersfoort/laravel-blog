...
<body>
    ...
    <div class="max-w-2xl mx-auto flex flex-col min-h-screen">
      ...
      <form action="/posts" method="post" class="space-y-8 mt-12">
        ...
        <div class="relative border border-gray-300 rounded-md px-3 py-2 shadow-sm focus-within:ring-1 focus-within:ring-indigo-600 focus-within:border-indigo-600 sm:w-[80%]">
          ...
          <input type="text" name="title" value="{{ $post->title }}" id="title" class="block border-0 p-0 text-gray-900 placeholder-gray-500 focus:ring-0 sm:text-sm" placeholder="Title of your blogpost">
        </div>
        ...
        <div class="relative">
          ...
          <div class="mt-1">
            <textarea rows="15" name="content" id="content" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-lg" placeholder="content of your blogpost. Text only.">{{ $post->content }}</textarea>
          </div>
        </div>
        ...
        <div class="">
          ...
          <a href="{{ route('posts.destroy', ['post' => $post]) }}" id="delete-link" class="text-center ml-2 inline-block font-semibold py-[0.875rem] px-6 text-red-500 bg-white rounded-xl border border-red-500 max-w-max transition-all duration-200 focus:bg-red-500 focus:text-white hover:bg-red-500 hover:text-white" onclick="event.preventDefault(); document.getElementById('delete-form').submit();">
            Delete
  ...
  <script>
    document.querySelector('#delete-link').addEventListener('click', (e) => {
      ...
      console.log(document.querySelector('#delete-form'));
